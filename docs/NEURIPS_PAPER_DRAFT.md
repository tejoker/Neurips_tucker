...
We detect anomalies as structural changes in the learned causal graph, classified via a three-metric state-aware system (absolute deviation, temporal change, trend), and explain them through a novel **Causal Path Tracing** algorithm that identifies fault propagation paths. On NASA Telemanom (SMAP/MSL, $d \approx 25-55$), we achieve an **$F_{1,max}$ score of 0.88** (Recall 0.80, Precision 0.99) and an **AUC-PR of 0.86**, outperforming TranAD's precision while providing human-interpretable causal explanations. Ablations further show strong compute-memory gains from Tucker compression, reported with reproducible script-based provenance.
...
**Statistical Testing**: Paired t-test across matched seeds/entities (5 seeds by default), with mean ± std and 95% confidence intervals (p<0.05).
...
This reveals a fundamental trade-off: The L1 sparsity penalty ($\lambda_{sparse}$) that enables our high-precision detection prevents the model from learning "dense" failure graphs where >50% of the system interacts. Effectively, the model selects a sparse subset of the active failure modes, which is sufficient for detection but incomplete for diagnosis of catastrophic failures. Future work will explore relax### 4.7 Ablation Studies: Tensor Rank Sensitivity

**Table 2: Component Ablation Results (Anomaly Detection Metrics on SMD, mean ± std across seeds/entities)**

| Variant | PA-F1 | Std-F1 | AUC-PR | Params | Time (s) |
|---------|-------|--------|--------|--------|----------|
| Tucker-CAM (Full) | see generated table | see generated table | see generated table | see generated table | see generated table |
| No Tucker (Dense) | see generated table | see generated table | see generated table | see generated table | see generated table |
| CP Decomposition | see generated table | see generated table | see generated table | see generated table | see generated table |
| Single Metric ($s_{abs}$ only) | see generated table | see generated table | see generated table | see generated table | see generated table |
| L1 instead of Top-K | see generated table | see generated table | see generated table | see generated table | see generated table |

To rigorously validate the theoretical computational bounds defined in Theorem 2, we conducted a dual-axis sensitivity analysis on the tensor ranks $R_w$ (contemporaneous) and $R_a$ (time-lagged). The full empirical results connecting the hyper-parameters to peak memory footprint and optimization time are detailed in **Appendix C.2**.
All ablation metrics, scripts, and provenance follow the canonical protocol in `docs/NEURIPS_ABLATION_PROTOCOL.md`.
...
4. **Rank Sensitivity**: Decoupling $R_w$ and $R_a$ proves crucial; increasing $R_w$ impacts computation cubically $O(R_w^3)$ while $R_a$ scales quartically $O(R_a^4)$. We report this trend with generated scaling plots and parameter/memory surrogates from the canonical rank study pipeline.
...
### C. Computational Insights and Resource Scaling

#### C.1 Setting and Configuration
- CPU: 64-core AMD EPYC 7742 (Parallel Execution via 8 workers x 8 threads)
- RAM: 125GB Limits enforced per node
- GPU: CPU-only formulation
- Code & Replication: Automated scripts generating empirical points found in `scripts/run_rank_ablation.sh` will be open-sourced.

#### C.2 Tensor Rank Parameter Sensitivity
We analyzed the effect of the tensor ranks on the objective F1 score relative to computational scaling (Time/Memory footprint) on the MSL dataset ($d=55$) and a large-scale Synthetic benchmark ($d=500$). The experiments decoupled the analysis between $R_w$ (contemporaneous graph) and $R_a$ (lagged associations).

**Results on MSL ($d=55$)**:
- **Grid Evaluation ($R \in \{5, 10, 20, 30, 40\}$)**: We found that setting $R_w > 40$ starts degenerating towards the input dimension $d$, violating the low-rank assumption and increasing memory footprint with no improvement to the $F_1$ anomaly detection measure.
- **Optimal Subspace**: A configuration of $(R_w=20, R_a=10)$ achieves $F1=0.88$, whereas compressing too aggressively ($R_w=5$) drops performance to $F1=0.74$.

**Results on High-Dimensional Data ($d=500$)**:
- **Scaling Complexity bounds**: Fixing $R_a=10$ and scaling $R_w \in \{10, 20, 50, 100, 200\}$ perfectly matches the theoretical bounds derived in Theorem 2. The computation time exhibited an empirical $O(d \cdot R_w^3)$ scaling, whereas the memory overhead scales linearly $O(d \cdot R_w)$.
- Please see `ablation_scaling_msl.pdf` (main appendix figure) and `ablation_scaling_highd.pdf` (high-dimensional synthetic supplement) generated by the rank analysis scripts.
