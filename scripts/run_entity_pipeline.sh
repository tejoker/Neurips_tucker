...
# ------------------------------------------------------------------
# Use user-provided python first, then common fallbacks
PYTHON_EXEC="${PYTHON_EXEC:-}"
if [ -z "$PYTHON_EXEC" ]; then
    if [ -x "$WORKSPACE_ROOT/.venv/bin/python3" ]; then
        PYTHON_EXEC="$WORKSPACE_ROOT/.venv/bin/python3"
    elif command -v python3 >/dev/null 2>&1; then
        PYTHON_EXEC="python3"
    else
        echo "ERROR: python3 not found. Set PYTHON_EXEC explicitly."
        exit 1
    fi
fi

# Fallback if explicit path is invalid
if [ ! -x "$PYTHON_EXEC" ] && [ "$PYTHON_EXEC" != "python3" ]; then
    echo "WARNING: Python not executable at $PYTHON_EXEC, falling back to python3"
    PYTHON_EXEC="python3"
fi
